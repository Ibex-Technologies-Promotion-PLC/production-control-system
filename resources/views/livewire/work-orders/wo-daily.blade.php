    ...
    <x-content theme="green">
        <x-slot name="header">
            <x-page-header icon="settings" header="workorders.daily_work_orders">
                <x-slot name="buttons">
                    <a href="{{route('work-orders.create')}}" wire:click="openWoFormModal" class="ui icon mini teal button"
            ...
            <div class="flex justify-between">
                <div class="font-bold text-sm">
                    @if ($this->inProgress)
                        ...
                        <span
                            class="text-ease">{{ __('workorders.started_at_time', ['time' => $this->inProgress->startedAt()->diffForHumans()]) }}</span>
                    @else
