<div>
    ...
    <x-content theme="purple">
        <x-slot name="header">
                        ...
                        <div class="ui mini icon buttons">
                            @if ($workOrder->isSuspended())
                                <button wire:key="unsuspend" wire:click.prevent="unsuspend()" class="ui mini basic button"
                                    data-tooltip="{{ __('common.suspended') }}" data-variation="mini"
                                    data-position="bottom right">
                                    <i class="gray circle icon"></i>
                                </button>
                            @elseif($workOrder->isCompleted())
                                <button class="ui mini basic button"
                                    data-tooltip="{{ __('workorders.production_is_completed') }}" data-variation="mini"
                                    data-position="bottom right">
                                    <i class="green checkmark icon"></i>
                                </button>
                            @elseif($workOrder->isInProgress())
                                <button class="ui mini basic button"
                                    data-tooltip="{{ __('workorders.production_continues') }}" data-variation="mini"
                                    data-position="bottom right">
                                    <i class="orange loading cog icon"></i>
                                </button>
                            @else
                                <button wire:key="suspend" wire:click.prevent="suspend()" class="ui mini basic button"
                                    data-tooltip="{{ __('common.active') }}" data-variation="mini"
                                    data-position="bottom right">
                            ...
                            @if (!$workOrder->isInProgress())
                                <button wire:click.prevent="openDeleteModal()" class="ui mini basic button"
                                    data-tooltip="{{ __('workorders.wo_delete') }}" data-variation="mini"
                                    data-position="bottom right">
            ...
            @else
                <x-page-header icon="project diagram" header="workorders.create.header"
                    subheader="workorders.create.subheader" />
            @endif
        </x-slot>
                ...
                <x-slot name="left">
                    @if ($editMode)
                        <x-dropdown model="product_id" triggerOnEvent="new_work_order_created"
                            dataSourceFunction="getProductsProperty" class="required" sClass="disabled search"
                    ...
                    @else
                        <x-dropdown model="product_id" triggerOnEvent="new_work_order_created"
                            ...
                            wire:ignore />
                    @endif
                    <x-input action model="wo_lot_no" label="validation.attributes.wo_lot_no"
                        placeholder="validation.attributes.wo_lot_no" class="required mini">
                        <x-slot name="action">
                    ...
                    <x-dropdown dataType="pointer" iModel="wo_amount"
                        ...
                        value="id" text="name" placeholder="{{ __('units.unit') }}" wire:ignore />

                    <x-datepicker noErrors :model="'wo_datetime'" :initialDate="$wo_datetime" :label="__('validation.attributes.wo_datetime')"
                        class="required field" />
                    <x-input noErrors model="wo_code" label="{{ __('validation.attributes.wo_code') }}"
                        placeholder="{{ __('validation.attributes.wo_code') }}" class="required field" />
                            ...
                            @if ($selectedProduct->recipe->ingredients->isEmpty())
                                <x-placeholder icon="red exclamation">
                                    <div class="text-sm">
                                        ...
                                        <div>
                                            {{ __('workorders.recipe_ingredients_must_be_correct_for_keep_inventory_flawless') }}
                                        </div>
                                        <div class="pt-5">
    ...
    @if ($editMode && $workOrder)
        <div x-data="{ deleteModal: @entangle('deleteModal') }" x-cloak>
            <x-confirm atConfirm="confirmDelete()" active="deleteModal" color="red"
                confirm="{{ __('common.delete') }}" deny="{{ __('common.cancel') }}">
                @if ($workOrder->isCompleted())
                    ...
                    <div class="text-xs text-ease-red pt-5">
                        {{ __('workorders.all_stock_moves_will_be_deleted_which_is_added_by_this_wo') }}
                    </div>
                @else
                    <div>{{ __('common.are_you_sure_you_want_to_delete') }}</div>
                @endif
