<x-content>
    ...
    <div x-data="{ newUserModal: @entangle('newUserModal') }">
        <x-custom-modal active="newUserModal">
            <div class="p-3">
                <h1 class="flex justify-center font-semibold text-2xl my-5">{{ __('users.create_user') }}</h1>
                ...
                <div class="ui mini form border p-2 rounded">
                    <x-input model="password" label="{{ __('users.user_password') }}"
                        placeholder="{{ __('users.user_password') }}" />
                </div>
                <div class="ui mini form border p-2 rounded">
                    <x-dropdown model="role" :label="$label ?? __('users.user_role')" :sId="$sId ?? 'selectRole'" :sClass="$sClass ?? 'search'" class="required"
        ...
        <div x-data="{ rolesModal: @entangle('rolesModal') }">
            <x-custom-modal active="rolesModal">
                <div class="p-3 border rounded shadow bg-white">
                    <x-dropdown-multiple
                        label="{{ __('roles.roles_for_user', ['user' => ucwords($selectedUser->name)]) }}"
                        model="roleIds" sId="role_select" class="mini">
