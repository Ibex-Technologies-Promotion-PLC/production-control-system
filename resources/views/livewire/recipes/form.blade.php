<div>
    ...
    <x-content theme="orange">
        ...
        <x-slot name="header">
            <x-page-header icon="mortar pestle" header="{{ $this->headerText()['main'] }}"
                subheader="{{ $this->headerText()['sub'] }}">
                <x-slot name="buttons">
                    <div class="ui mini icon buttons">
                        @if ($this->isLocked())
                        <button wire:click.prevent="unlock()" class="ui mini gray basic button"
                            data-tooltip="{{ __('common.unlock') }}" data-variation="mini"
                        ...
                        <button wire:click.prevent="restoreForm()" class="ui mini basic  button"
                            data-tooltip="{{ __('common.undo_changes') }}" data-variation="mini"
                            data-position="bottom right">
                        ...
                        <button wire:click.prevent="openDeleteConfirmModal()" class="ui mini basic button"
                            data-tooltip="{{ __('recipes.delete_recipe') }}" data-variation="mini"
                            data-position="bottom right">
            ...
            <form class="ui mini form">
                        ...
                        <x-input action model="rcp_code" label="validation.attributes.rcp_code"
                            ...
                            <x-slot name="action">
                                <button wire:click.prevent="suggestCode"
                                    class="ui teal mini right labeled icon button @if (!$selectedProduct) disabled @endif">
                ...
                @if ($this->producibles->count() <= 0)
                    <div class="pt-2 font-semibold text-sm">!!! There are no products in the list, please <a
                        class="text-red-600" href="{{ route('products.create') }}">Start Here</a>...
        </div>
        @endif
        </form>
</div>
        ...
        <div class="flex flex-col gap-6 fields">
            ...
            <div wire:key="wire_card{{ $key }}"
                ...
                <div
                    ...
                    @if ($this->isLocked())
                    <span wire:key="locked.{{ $key }}"
                        data-tooltip="{{ $this->literalTooltip($key) }}" data-variation="mini"
                        data-position="top left">
                        <i class="{{ $this->literalClass($key) }}"></i>
                    </span>
                    @else
                    <span wire:key="unlocked.{{ $key }}"
                        wire:click="toggleLiteral({{ $key }})"
                        data-tooltip="{{ $this->literalTooltip($key) }}" data-variation="mini"
                ...
                <div
                    ...
                    <div class="flex flex-col">
                        <div class="flex gap-2 items-center">
                            ...
                            <div class="text-sm text-ease">({{ $card['ingredient']['prd_code'] }})
                            </div>
                        </div>
                        <div
                            ...
                            <i wire:loading.class="animate-spin orange circle notch icon"
                                wire:target="cards.{{ $key }}.unit_id, cards.{{ $key }}.amount"></i>
                        </div>
                    </div>
                    ...
                    <div
                        ...
                        <span
                            class="text-sm">{{ $this->getIngredientUnit($card)['name'] }}</span>
                    </div>
                    @else
                    <div class="flex gap-2 items-center">
