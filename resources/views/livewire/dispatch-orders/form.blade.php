    ...
    <x-content>
        ...
        <form class="ui mini form" wire:submit.prevent="submit">
            <x-form-divider bottomClass="bg-gray-100 shadow-inner">
                ...
                <x-slot name="left">
                    <x-dropdown model="company_id" :collection="$this->companies" value="id" text="cmp_name,cmp_current_code"
                        ...
                        <x-dropdown dataType="pointer" dataSourceFunction="getCompanyAddress" model="address_id" dataSource="companyAddresses" value="id"
                    ...
                    <x-input defer model="do_number" label="{{ __('validation.attributes.do_number') }}"
                        placeholder="{{ __('validation.attributes.do_number') }}" class="required" noErrors />
                    ...
                    <div x-data="{ salesTypeModal: false }" class="equal width fields">
                        <x-dropdown model="sales_type_id" triggerOnEvent="st_updated"
                            ...
                            sClass="search" class="required" label="{{ __('validation.attributes.sales_type_id') }}"
                            placeholder="{{ __('validation.attributes.sales_type_id') }}" sId="sales_type_id" noErrors>
                            <x-slot name="right">
                                <div class="pt-0.5 flex justify-end gap-2 items-center" id="BUTTONS">
                                    @if ($sales_type_id)
                                        <span wire:key="stDelete" wire:click.prevent="stDelete({{ $sales_type_id }})"
                                            class="cursor-pointer text-red-400 hover:text-red-600 "
                                            data-tooltip="{{ __('common.delete') }}" data-variation="mini">
                                            <i class="trash icon"></i>
                                        </span>
                                        <span wire:key="stEdit" wire:click.prevent="stEdit({{ $sales_type_id }})"
                                    ...
                                    <span wire:key="stAdd" @click="salesTypeModal = true"
                                        ...
                                        data-tooltip="{{ __('common.add') }}" data-variation="mini">
                                        <i class="plus icon"></i>
                                    </span>
                                </div>
                            </x-slot>
                        ...
                        <x-custom-modal active="salesTypeModal" theme="green"
                            header="{{ __('dispatchorders.add_sales_type') }}">
                        ...
                        <div class="w-full h-full border bg-gray-100 rounded flex items-center justify-center"
                            ...
                            <div class="text-center">
                                ...
                                <div class="pt-2 text-xs text-ease">
                                    {{ __('dispatchorders.plate_number_driver_expenses_etc') }}</div>
                            </div>
                        </div>


                        <div x-show="extrasField">
                            <x-input defer model="de_license_plate"
                                label="{{ __('validation.attributes.de_license_plate') }}"
                                placeholder="{{ __('validation.attributes.de_license_plate') }}" noErrors />
                            <x-input defer model="de_driver_name"
                                label="{{ __('validation.attributes.de_driver_name') }}"
                                placeholder="{{ __('validation.attributes.de_driver_name') }}" noErrors />
                            <x-input defer model="de_driver_phone"
                                label="{{ __('validation.attributes.de_driver_phone') }}"
                                placeholder="{{ __('validation.attributes.de_driver_phone') }}" noErrors />
                            <x-input defer model="de_dispatch_expense"
                                label="{{ __('validation.attributes.de_dispatch_expense') }}"
                                placeholder="{{ __('validation.attributes.de_dispatch_expense') }}" noErrors />
                            <x-input defer model="de_handling_expense"
                                label="{{ __('validation.attributes.de_handling_expense') }}"
                                placeholder="{{ __('validation.attributes.de_handling_expense') }}" noErrors />
