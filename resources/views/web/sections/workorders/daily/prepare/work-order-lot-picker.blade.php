@if ($woLotPickerModal)
    <div x-data="{ woLotPickerModal: @entangle('woLotPickerModal') }">
        <x-custom-modal active="woLotPickerModal">
            ...
            @if ($selectedIngredient->getIsInStockAttribute())
                <div class="bg-white" wire:key="do_{{ $selectedIngredient }}">
                        ...
                        <div>
                            ...
                            <span class="text-green-600">
                                ...
                                <span class="text-ease">
                                    {{ $this->getToBase() }}
                                    {{ $selectedIngredient->getBaseUnitAttribute()->name }}
                                </span>
                            </span>
                        </div>
                        <div>
                    ...
                    <div class="p-6 shadow-md flex flex-col gap-8 md:gap-4">
                        @foreach ($rows as $key => $row)
                            <div wire:key="row_{{ $key }}"
                                class="flex flex-col md:flex-row gap-3 border border-dashed p-3 rounded-lg hover:border-orange-400 ease-in-out duration-200">
                                ...
                                <select class="form-select text-sm flex-1"
                                    wire:model.prevent="rows.{{ $key }}.lot_number">
                                    <option value="" selected>{{ __('dispatchorders.select_lot_number') }}
                                    </option>

                                    @foreach ($selectedIngredient->getLotsAttribute() as $index => $lot)
                                        <option value="{{ $lot['lot_number'] }}">
                                            <span> {{ $lot['lot_number'] }}</span> |
                                            <span class="text-xs">{{ __('common.available') }}:
                                                {{ $lot['available_amount_string'] }}</span>
                                            @if ($lot['reserved_amount'])
                                                | {{ __('workorders.reserved') }}:
                                ...
                                <div class="flex gap-4">
                                    @if ($this->inputDisabled($key))
                                        <x-input wire:key="reservedamountinput_{{ $key }}" type="number"
                                            model="rows.{{ $key }}.reserved_amount"
                                            placeholder="{{ __('common.amount') }}"
                                            innerLabel="{{ $selectedIngredient->getBaseUnitAttribute()->name }}"
                                            iClass="disabled" noErrors class="ui tiny input flex-1" />
                                    @else
                                        <x-input wire:key="reservedamountinput_{{ $key }}" type="number"
                                            model="rows.{{ $key }}.reserved_amount"
                                            placeholder="{{ __('common.amount') }}"
                                            innerLabel="{{ $selectedIngredient->getBaseUnitAttribute()->name }}"
                                            noErrors class="ui tiny input flex-1" />
                                    @endif
                                    <div class="flex items-center w-1/12 justify-center">
                                        <i wire:click="removeRow({{ $key }})"
                                            class=" cancel red icon @if ($this->cannotRemoveRow()) disabled @else cursor-pointer link @endif"></i>
                                    </div>
                                </div>
                            </div>
                        @endforeach
                    ...
                    <div class="flex p-3">
                        <button wire:click="submitLots()"
                            class="ui mini orange w-full button @if ($this->cannotSubmit()) disabled @endif">
                            {{ __('common.save') }}
                        </button>
                        <button wire:click="addRow()"
                            class="ui mini icon button @if ($this->cannotAddRow()) disabled @endif">
