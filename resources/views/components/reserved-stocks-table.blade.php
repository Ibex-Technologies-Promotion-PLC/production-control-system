        ...
        <x-tbody>
            @forelse($reservations as $reservation)
                <x-table-row class="text-ease hover:bg-cool-gray-100">
                    ...
                    <x-tbody-item class="font-bold right aligned text-blue-700">{{ number_format($reservation->reserved_amount, 2) }} {{ $reservation->product->getBaseUnitAttribute()->name }}</x-tbody-item>
                </x-table-row>
            @empty
